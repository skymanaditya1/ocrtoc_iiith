...
def get_match(obj_list, images, camera_poses, camera_matrix, superglue_config, rot = 0, model_suffix, camera_Twc):
    ...
    segment_image = cv2.imread(segmeentimagepath)
    original_image = copy.deepcopy(segment_image)
    resize = config['resize']
    ret = {}
    ...
    template_name = '000001'
    template_name_features = template_name + '_features.pickle'
    template_name_image = template_name + '.png'

    for obj in tqdm(obj_list, 'Checking match for one object'):
        ...
        cache_feature_path = os.path.join(base_dir, template_name_features)
           
        t_image, t_inp, t_scale = read_image(os.path.join(
                    base_dir, template_name_image), 'cuda', resize, 0, False)
