...
def ros_to_pcl(ros_cloud, color = True):
    ...
    """
    # pc_np = ros_numpy.point_cloud2.pointcloud2_to_xyz_array(ros_cloud, remove_nans=True)
    # # print(pc_np.shape)
    # pcl_data = pcl.PointCloud(np.array(pc_np, dtype=np.float32))
...
def pcl_to_ros(pcl_array, color = True):
    """ Converts a ROS PointCloud2 message to a pcl PointXYZRGB
    ...
    print(pcl_array)
    if color == False:
        pcl_array = XYZ_to_XYZRGB(pcl_array, color=[255,0,0])
        # print(pcl_array)
    ros_msg = PointCloud2()

    ros_msg.header.stamp = rospy.Time.now()
    ros_msg.header.frame_id = "depth_camera_link" #"world"
